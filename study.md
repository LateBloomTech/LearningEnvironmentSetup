...
## 基本情報技術者
11/14 AM
if elseを実際に動かして理解を行っていく。
11/13 PM
if else辺りの疑似言語の理解が及ばず、明日再度取り組みます。
11/13 AM
木構造について、また1－2－2アルゴリズムの流れ図まで学習を行う。
ブランチとか何かGITのコマンドで有ったような。
疑似言語基本的な処理、関数、変数、注釈、if elseif  else endif while endwhile do while for endfor 大体の言語に似た表現あったような。
ベンチマーク.py修正と言うか作りなおしの方が早いかもと言う話が・・・・
自身のレベルに合わせて作りなおしますか・・・・・

11/12 PM
1-2-1 データとデータ構造リストまで終了。変数とか行列とかのお話し。変数は意味は理解できるけど実際に扱うと少し混乱する。
WSLCPUベンチ分からない、ただWINDOWS11Homeでリモートデスクトップ接続するとなるとツールを活用する方向になると、ただそうなるとセキュリティ上よろしくないと有識者より、アドバイスがありました。
またroot権限がWSLにない状況で、掲載ページのLinuxCPUベンチマークコードは、Ubuntuから直接Linuxを動かしている為、そのままでは自分の環境で動かす事は難しい。とアドバイスも。
色々自身が想定していた方向とずれが生じていますが、可能な限り自身が思い描いた方向で事を進めるように、CPUのpyファイルの修正を行って行きたいです。
...
1－1－5
制御に関する理論
全体的にアナログデータを→デジタル→アナログの変換する理論である、通信に関しては、データ転送の際の誤り検出や訂正する誤り制御方式が面白いと感じた。
WSLCPUベンチマーク、JSON出力確認できるも、コマンドが見つかりませんと、WSLCLに見える。
元のサイトを読み込んで、自身の環境違いを確認していく。
...
特に待ち行列理論は、以下にシステムを活用して、業務を効率化出来たか数字で表現できるため、今後活用する事が多くなってくると考える。
ローカルLLM、音声認識との連携考慮しましたが、Docker上での音声認識コンテナの起動まで行えましたが、そこからは音声認識モデル設定あやふやな感じに。
リポジトリ作成して、動画制作も終わって公開したので、一段落と言う事で、Dockerの基礎部分であるWSLとpythonのベンチマークを見直す方向に、とりあえず56Line周囲のコードでエラーが出ていたので書き直したら、エラー出ずに起動しております。
このまま、動いて最後まで完走してくれたら助かります。ベンチマーク時間掛かるので明日確認する形とします。
